...
COMMENT ON COLUMN BOARD_INFO.REPLY_CNT IS '댓글개수';
--시퀀스
...
CREATE TABLE "REPLY_INFO"
   ("REPLY_NO" NUMBER NOT NULL ENABLE,
	"BOARD_NO" NUMBER NOT NULL ENABLE,
	"REPLY_CONTENT" VARCHAR2(100) NOT NULL ENABLE,
	"USER_ID" VARCHAR2(20) NOT NULL ENABLE,
	"DEL_YN" CHAR(1) DEFAULT 'N' NOT NULL ENABLE,
	"REG_DATE" DATE DEFAULT SYSDATE NOT NULL ENABLE,
	"MOD_DATE" DATE DEFAULT SYSDATE,
	 CONSTRAINT "REPLY_INFO_PK" PRIMARY KEY ("REPLY_NO","BOARD_NO")
)
;
COMMENT ON COLUMN REPLY_INFO.REPLY_NO IS '댓글 순번(시퀀스:REPLY_NO_SEQ)';
COMMENT ON COLUMN REPLY_INFO.BOARD_NO IS '게시글 순번';
COMMENT ON COLUMN REPLY_INFO.REPLY_CONTENT IS '댓글 내용';
COMMENT ON COLUMN REPLY_INFO.USER_ID IS '댓글 작성 아이디';
COMMENT ON COLUMN REPLY_INFO.DEL_YN IS '삭제여부';
COMMENT ON COLUMN REPLY_INFO.REG_DATE IS '등록일';
COMMENT ON COLUMN REPLY_INFO.MOD_DATE IS '수정일';
--시퀀스
CREATE SEQUENCE REPLY_NO_SEQ INCREMENT BY 1 MINVALUE 1 MAXVALUE 99999999 CYCLE NOCACHE ORDER ;
